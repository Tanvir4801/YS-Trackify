    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3 text-sm text-text-muted">
      <div className="flex items-center gap-2">
        ...
        <select
          ...
          className="h-8 rounded-md border border-border-strong bg-bg-input px-2 text-xs shadow-sm outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 text-text-primary"
        >
      ...
      <div className="flex items-center gap-1">
        ...
        {windowPages.map((p) => (
          <Button
            ...
            size="sm"
            onClick={() => setPage(p)}
            className={`h-8 w-8 p-0 text-xs ${p === page ? 'bg-gold text-bg-primary font-semibold hover:opacity-90' : ''}`}
          >
            {p + 1}
          </Button>
        ))}
