...
class ReportService {
  ...
  Future<void> fetchMonthData(int month, int year) async {
    ...
    try {
      ...
      Future<void> storePaySnap(QuerySnapshot<Map<String, dynamic>> snap) async {
  ...
  List<Map<String, dynamic>> generateSalaryReport(int month, int year) {
    ...
    return labours.map((labour) {
      ...
      final daysThreeQuarter = records.where((a) => a.status == AttendanceStatus.threeQuarter).length;
      final daysHalf    = records.where((a) => a.status == AttendanceStatus.half).length;
      final daysQuarter = records.where((a) => a.status == AttendanceStatus.quarter).length;
      final daysAbsent  = records.where((a) => a.status == AttendanceStatus.absent).length;
      // Pending records are excluded from wage totals (wageFactor = 0).
      final totalDays   = records.fold<double>(0.0, (acc, a) => acc + a.effectiveShiftFactor);
      ...
      return {
        ...
        'daysThreeQuarter': daysThreeQuarter,
        'daysHalf':     daysHalf,
        'daysQuarter':  daysQuarter,
  ...
  List<Map<String, dynamic>> generateSiteWiseReport(int month, int year) {
    ...
    return bySite.entries.map((entry) {
      ...
      final threeQuarterCount = recs.where((r) => r.status == AttendanceStatus.threeQuarter).length;
      final halfCount    = recs.where((r) => r.status == AttendanceStatus.half).length;
      final quarterCount = recs.where((r) => r.status == AttendanceStatus.quarter).length;
      final absentCount  = recs.where((r) => r.status == AttendanceStatus.absent).length;
      // Pending records are excluded from wage totals (same 0-factor as absent).

      final totalWage = recs.fold<double>(0, (sum, r) {
        return sum + (r.wageAtTime * r.effectiveShiftFactor);
      });
      ...
      return {
        ...
        'threeQuarterCount': threeQuarterCount,
        'halfCount':    halfCount,
  ...
  List<Map<String, dynamic>> generateLabourWiseReport(
    ...
    return records.map((a) => {
      ...
      'siteId':       a.siteId,
      'earned':       a.wageAtTime * a.effectiveShiftFactor,
    }).toList();
  }
  ...
  Future<String> exportToExcel(int month, int year) async {
    ...
    salarySheet.appendRow([
      ...
      TextCellValue('Wage At Time'), TextCellValue('Present'), TextCellValue('3/4 Day'),
      TextCellValue('Half Day'), TextCellValue('1/4 Day'),
    ...
    for (final row in salaryData) {
      salarySheet.appendRow([
        ...
        IntCellValue(row['daysThreeQuarter'] as int),
        IntCellValue(row['daysHalf'] as int),
        IntCellValue(row['daysQuarter'] as int),
