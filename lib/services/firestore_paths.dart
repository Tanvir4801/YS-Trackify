...
class FirestorePaths {
  ...
  static DocumentReference<Map<String, dynamic>> attendanceRecordRef(
    ...
    String siteId = '',
      ...
      attendanceRecordsCol(contractorId, dateKey)
          .doc(siteId.isNotEmpty ? '${labourId}_$siteId' : labourId);

  // ── Closing Reports ────────────────────────────────────────────────────────

  static CollectionReference<Map<String, dynamic>> closingReportsCol(
    String contractorId,
  ) =>
      _db.collection('closingReports').doc(contractorId).collection('reports');

  static DocumentReference<Map<String, dynamic>> closingReportDoc(
    String contractorId,
    String reportId,
  ) =>
      closingReportsCol(contractorId).doc(reportId);
}
