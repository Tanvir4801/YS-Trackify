...
class _ScannerScreenState extends State<ScannerScreen>
  ...
  Future<void> _onDetect(BarcodeCapture capture) async {
    TelemetryService.instance.trackFeatureUsage('Scan QR');
    ...
    // Step 2: Valid QR — Check capacity
    final summaries = await _attendanceService.getDailyShiftSummary(_todayString());
    final summary = summaries[precheck.labourId] ?? 
        DailyLabourSummary(labourId: precheck.labourId, totalShiftFactor: 0.0, siteVisits: []);
    
    final remainingFactor = 1.0 - summary.totalShiftFactor;
    ...
    if (remainingFactor <= 0) {
      ...
      setState(() {
        _lastResult = ScanResult(
          ...
          labourName: precheck.labourName.isNotEmpty ? precheck.labourName : null,
        );
        _isProcessing = false;
      });
      _loadPendingCount();
    ...
    // Step 3: Valid QR and has capacity — play success sound then show type sheet
    await _scannerService.playSuccessFeedback();
    if (!mounted) return;
