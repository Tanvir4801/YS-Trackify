...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _checkAuth() async {
    final minimumDelay = Future<void>.delayed(const Duration(milliseconds: 1700));
    
    VersionCheckResult? versionResult;
    try {
      versionResult = await VersionService.instance.checkVersion().timeout(const Duration(seconds: 5));
    } catch (_) {}

    AuthResult? result;
    try {
      result = await AuthService().checkCurrentUser().timeout(
    ...
    if (versionResult != null && !versionResult.isValid) {
      showDialog(
        context: context,
    ...
    if (result.role == 'super_admin') {
      Navigator.of(context)
          .pushNamedAndRemoveUntil('/super-admin', (r) => false);
      return;
    }
    if (result.role == 'supervisor' || result.role == 'admin') {
      Navigator.of(context)
          .pushNamedAndRemoveUntil('/supervisor-home', (r) => false);
      return;
    }
    if (result.role == 'labour') {
      Navigator.of(context)
          .pushNamedAndRemoveUntil('/labour-home', (r) => false);
      return;
    }
    Navigator.of(context)
        .pushNamedAndRemoveUntil('/unauthorized', (r) => false);
  }
                      ...
                      FadeTransition(
                        ...
                        child: ScaleTransition(
                          ...
                          child: Container(
                            ...
                            child: const Icon(Icons.architecture_rounded, size: 52, color: AppColors.gold),
                          ),
                        ),
                      ),
