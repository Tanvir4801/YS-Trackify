  ...
  void _rebuild() {
    ...
    for (final entry in _flatDocs.entries) {
      ...
      if (_matchesMonth(date, monthStr)) {
        attendance[entry.key] = data; // use document id as key
      }
    }
    ...
    for (final data in attendance.values) {
      ...
      final factor = (data['shiftFactor'] as num?)?.toDouble() ?? 
                     (status == 'present' ? 1.0 : (status == 'half' ? 0.5 : 0.0));
      final ot = (data['overtimeHours'] as num?)?.toDouble() ?? 0;
      
      if (factor == 1.0) {
        daysPresent++;
      } else if (factor > 0) daysHalf++; // Anything fractional > 0 treated as 'half' or partial for UI display
    ...
    final workedDays  = attRecords.fold<double>(0, (sum, r) {
      final status = _normalizeStatus(r['status']);
      return sum + ((r['shiftFactor'] as num?)?.toDouble() ?? (status == 'present' ? 1.0 : (status == 'half' ? 0.5 : 0.0)));
    });
